        ...
        public async Task ExecuteAsync(string[] args, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (!hasWizard)
                {
                    ...
                    {
                        throw new InvalidDataException(MessageNotValidStatusForUpdate(infoadr.Header.StatusUpdate));
                    }
                ...
                await _pluginManager.DispatchAsync(AdrEventType.Approved, record.ToSnapshot(), infoadr.FileName, () => content, repoconfig.ToSnapshot(), isReplay: false, isActive: isActive, cancellationToken: cancellationToken);
            }
            catch (Exception ex)
            {
        ...
        private async Task<Dictionary<Arguments, string>> ApproveAdrWizard(CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                (bool, string?) validselect(AdrFileNameComponents info)
                {
                    ...
                    {
                        return (false, MessageNotValidStatusForUpdate(info.Header.StatusUpdate));
                    }
