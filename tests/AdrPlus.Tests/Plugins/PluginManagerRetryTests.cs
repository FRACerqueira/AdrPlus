...
public class PluginManagerRetryTests
{
    ...
    public async Task RetryPendingAsync_WhenHookSucceedsOnFirstAttempt_RemovesEntry()
    {
        ...
        var summary = await manager.RetryPendingAsync(ResolverFor("0001-v1-r0"), CreateRepoSnapshot(), TestContext.Current.CancellationToken);
    ...
    public async Task RetryPendingAsync_WhenAdrNoLongerResolves_DropsEntry()
    {
        ...
        var summary = await manager.RetryPendingAsync(_ => null, CreateRepoSnapshot(), TestContext.Current.CancellationToken);
