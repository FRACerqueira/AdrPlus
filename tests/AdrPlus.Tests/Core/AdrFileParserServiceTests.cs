    ...
    public class AdrFileParserServiceTests
    {
        ...
        public async Task ParseAdrHeaderAndContentAsync_WithEmptyStatusLines_ParsesSuccessfully()
        {
            ...
            var lines = new[]
            {
                ...
                "##### Status",
                "- Proposed (2025-04-17)",
                $"- {AppConstants.AdrEmptyStatusMarker}",
                $"- {AppConstants.AdrEmptyStatusMarker}",
                "# Test ADR Title",
                "---",
                "This is the content of the ADR."
        ...
        [Fact]
        public async Task ParseAdrHeaderAndContentAsync_WithScopeDomain_ExtractsBothCorrectly()
        {
            // Arrange
            var filePath = "test.md";
            var lines = new[]
        ...
        [Fact]
        public async Task ParseAdrHeaderAndContentAsync_WithProjectScope_ExtractsScopeDomainCorrectly()
        {
            // Arrange
            var filePath = "test.md";
            var lines = new[]
            ...
            Assert.Equal("PaymentModule", header.Domain);
        }

        [Fact]
        public async Task ParseAdrHeaderAndContentAsync_WithMultipleScopeValues_ValidatesAgainstConfigScopes()
        {
            // Arrange
            var filePath = "test.md";
            var customConfig = new AdrPlusRepoConfig
            {
                ...
                LenVersion = 2,
                LenScope = 1,
                Scopes = "Enterprise;Project;Domain"
            };

            var scopeValues = new[] { "Enterprise", "Project", "Domain" };
            ...
            foreach (var scope in scopeValues)
            {
                ...
                // Assert
                Assert.True(header.IsValid, $"Failed for scope: {scope}");
                Assert.Equal(scope, header.Scope);
                Assert.Equal("TestDomain", header.Domain);
            }
        }

        [Fact]
        public async Task ParseAdrHeaderAndContentAsync_WithoutLenScope_StillParsesScopeFromHeader()
        {
            ...
            // Act
            var (header, _) = await _parser.ParseAdrHeaderAndContentAsync(filePath, configNoScope, _fileSystemService);

            // Assert - Even with LenScope = 0, scope and domain are still parsed from the header
            Assert.True(header.IsValid);
            Assert.Equal("Enterprise", header.Scope);
            Assert.Equal("TestDomain", header.Domain);
        }
        ...
        public async Task ParseAdrHeaderAndContentAsync_WithLongDomainName_ExtractsDomainCorrectly()
        {
            ...
            var lines = new[]
            {
                ...
                "##### Version: 1",
                "##### Revision: 1",
                "##### Enterprise:PaymentProcessingService",
                "##### Status",
                "- Proposed (2025-04-17)",
                $"- {AppConstants.AdrEmptyStatusMarker}",
                $"- {AppConstants.AdrEmptyStatusMarker}",
