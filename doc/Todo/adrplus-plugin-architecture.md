...
> **Status**: Implemented (v1). All decisions tagged Essential are built and tested; D23/D24 are rejected outright (not deferred); D29's CI-authoritative alternative is likewise rejected outright, not deferred — local/per-developer execution is the permanent model; D31–D33 (§3) are post-v1 refinements shipped after the original 11 implementation phases — see `adrplus-plugin-implementation-plan.md`'s addendum. **D34–D35 (2026-08-03) were implemented once against a per-repo `./plugins/<name>/` model, committed as a checkpoint, then D2/D29/D31/D35 were revised the same day (D36) to pivot discovery to a host-global store — see D36 for the pivot and D35's row for the post-pivot behavior it now describes.** The checkpoint commit (`fff9de7`) reflects the pre-pivot model; the pivot itself is spec-only until implemented against D36.
