...
| D35 | Plugin distribution via zip: `--install`/`--uninstall` (implemented 2026-08-03) | Adds `--install <path-to-zip>`/`--uninstall <name>` to `adrplus plugins`, as a convenience wrapper around D2's existing folder-based discovery (`./plugins/<name>/`) — not a new discovery mechanism, and manual copy remains equally valid. Zip must be named `<name>-<version>.zip`; its contents are exactly what already belongs in `./plugins/<name>/` today (dll, resolved deps, `plugin.json`, loose assets) — no new packaging format. The filename's `<name>` becomes the destination folder name (not the manifest `Name`, which may legitimately differ — see the `AdrIndexer`/`adr-indexer` precedent); the filename's `<name>`/`<version>` are cross-checked against the `plugin.json` found inside the zip, failing fast on mismatch. Every zip entry is guarded against path traversal before extraction, mirroring Phase 3's `entryAssembly` guard. Local file path only — no URL/registry fetch (stays out of scope, §14). Destination-exists policy: refuses without `--force`; with `--force`, overwrites everything unconditionally, including `plugin.json` and `state/` — an explicit, accepted trade-off (loses local `settings` customization and any pending retry state on a forced upgrade), chosen for implementation simplicity over a surgical merge. Never touches `activeplugins` on install (D31's trust checkpoint: a downloaded zip must never make a plugin start dispatching on its own — activation is a separate, scriptable step via D34). `--uninstall <name>` deletes `./plugins/<name>/` recursively **and** removes `name` from `activeplugins` — unlike install, cleanup has no trust question, and leaving a stale entry would only produce a permanent, useless `Missing` warning. Runs the existing `--validate` check against the freshly installed plugin and prints its result immediately. Prints the zip's SHA256 so the operator can optionally add it to the already-existing (but unenforced) allowlist `Hash` field. | Essential |
