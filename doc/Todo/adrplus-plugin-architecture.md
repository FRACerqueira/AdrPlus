...
| D37 | Test-harness factories for plugin authors (2026-08-03) | `AdrPlus.Abstractions` gains a new `AdrPlus.Abstractions.Testing` namespace (`AdrRecordSnapshotFactory`, `RepoInfoSnapshotFactory`, `AdrEventContextFactory`) — static `Create(...)` methods with sensible defaults for every parameter, so a plugin author's own unit test can get a fully valid `AdrEventContext` (including its nested `AdrRecordSnapshot`/`RepoInfoSnapshot`) in one call instead of hand-filling every `required` field, as the project's own `AdrPluginBaseTests.CreateContext()` fixture already had to do. Deliberately the narrower half of the original "Plugin SDK" roadmap bullet (§14) — the `dotnet new` template half was rejected outright for requiring a separate distribution artifact (see §14), while this half needed **zero new packages and zero new dependencies**: `AdrPlus.Abstractions` had no runtime `PackageReference` before this and still has none: the factories are plain C# object construction, not a testing framework, kept in their own namespace (not mixed into the production DTO types themselves) so it's clearly opt-in and doesn't imply the main package now depends on any test framework. Ships in the same package/version as the rest of `AdrPlus.Abstractions` — no separate `.Testing` package was judged worth the extra release-pipeline overhead for the one factory-class use case that exists today (same "not until a real plugin author needs it" judgment already applied elsewhere this session, e.g. the shelved plugin-edit-via-wizard feature). | Essential |
...
// AdrPlus.Abstractions.Testing (D37) — factories for a plugin author's own unit tests, not used by the host:
AdrRecordSnapshotFactory  // AdrRecordSnapshotFactory.Create(...) with sensible defaults
RepoInfoSnapshotFactory   // RepoInfoSnapshotFactory.Create(...) with sensible defaults
AdrEventContextFactory    // AdrEventContextFactory.Create(...) with sensible defaults
```
...
- **`dotnet new adrplus-plugin` project template — rejected outright 2026-08-03, not deferred.** A `dotnet new` template package is structurally a content package (`.template.config/template.json` + files to copy), not a compiled library — it cannot be folded into `AdrPlus.Abstractions` or the `AdrPlus` tool itself without conflating two different NuGet package shapes. Building it would mean standing up a **fourth** distribution artifact (after `AdrPlus`, `AdrPlus.Abstractions`, and the in-repo-only `AdrPlus.Plugins.AdrIndexer`), with its own package id, its own `.template.config`, and its own release pipeline mirroring `abstr-v*`. With zero external plugin authors today (only the in-repo `AdrIndexer` exists), that infrastructure cost isn't justified — see D37 for the one part of this original bullet (the mock test harness) that *was* judged worth building now, since it required none of that infrastructure.
